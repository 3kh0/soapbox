    ...
    <div class="flex flex-col items-center justify-center min-h-[50vh] p-4 font-sans">
      <div class="max-w-md w-full text-center space-y-8">
        ...
        <div v-else class="space-y-4">
          <h1 class="text-9xl font-black text-red-500 opacity-20 select-none leading-none">{{ error?.statusCode || '500' }}</h1>
          <div class="relative -mt-16">
             <h2 class="text-3xl font-bold text-white mb-3">Aw snap!</h2>
             <p class="text-dark-300 text-lg">{{ error?.message || 'Something went wrong.' }}</p>
          </div>
        </div>
