...
const s = ref(false); // fire at will
    ...
    <header class="border-b border-dark-700 bg-dark-800/50 backdrop-blur-sm sticky top-0 z-10">
      <NuxtLink to="/" class="block">
        <div class="max-w-5xl mx-auto px-6 py-4">
          ...
          <p class="text-red-400 text-sm mt-1">admin abooze</p>
        </div>
      </NuxtLink>
    </header>

    <div class="flex flex-1 max-w-7xl mx-auto w-full gap-6 px-6 py-6">
      <aside :class="['fixed md:static inset-0 md:inset-auto md:w-64 md:block', s ? 'block' : 'hidden']" class="bg-dark-800 md:bg-transparent rounded-lg md:rounded-none border md:border-0 border-dark-700 p-6 md:p-0">
        <nav class="space-y-3">
        ...
        <div class="mt-8 pt-6 border-t border-dark-700">
          ...
          <button @click="d" class="block w-full text-left px-4 py-2 rounded-lg hover:bg-dark-700 text-dark-400 hover:text-red-400 transition-colors text-sm">destroy session</button>
        </div>
      </aside>
